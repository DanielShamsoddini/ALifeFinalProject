...
class NEURAL_NETWORK: 
    ...
    def Update(self):
        for neuronn in self.neurons:
            if self.neurons[neuronn].Is_Sensor_Neuron():
                self.neurons[neuronn].Update_Sensor_Neuron()
            else:
                self.neurons[neuronn].Update_Hidden_Or_Motor_Neuron(self.neurons, self.synapses)

    def Get_Neuron_Names(self):
        return self.neurons.keys()

    def Is_Motor_Neuron(self, mname):
        return self.neurons[mname].Is_Motor_Neuron()

    def Get_Motor_Neurons_Joint(self, mname):
        return self.neurons[mname].Get_Joint_Name()

    def Get_Value_Of(self, mname):
        return self.neurons[mname].Get_Value()
